...
type API interface {
	...
	TraceBlock(blocknumber, address string) ([]utils.TraceResult, error)
	TransactionByHash(txid string) (*Transaction, error)
	TransactionReceipt(txid string) (*Transaction, error)
}
